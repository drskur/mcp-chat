...
async function processGraphStream(
  ...
  graphStream: AsyncIterable<[string, any]>,
  accStr: string,
...
// 새 메시지 스트리밍 액션
export const streamNewChatAction = action(
  async (input: ChatMessageInput): Promise<ReadableStream<ChatStreamChunk>> => {
    "use server";
...
// Resume 스트리밍 액션
export const streamResumeChatAction = action(
  async (input: {
    sessionId: string;
    streamId: string;
  ...
  return new ReadableStream<ChatStreamChunk>({
    async start(controller) {
      ...
      try {
        ...
        const graphStream = await graph.stream(graphInput, {
